...
for(i in unique(averaged_full_data_C0$image)){
  ...
  matrix_data <- as.matrix(data)
  file = i
  db <- dbscan(matrix_data, eps = 45)
  assign(paste("db_",file,sep = ""),db)
}

#list_of_eps_values = (10,10)

subset_data <- as.matrix(subset(averaged_full_data_C0,  averaged_full_data_C0$image == "abstract_2", select = c(PositionX, PositionY)))

#test_data <- subset(averaged_full_data_C0,  averaged_full_data_C0$image == #"fig_1", select = c(PositionX, PositionY))

#kNNdistplot(subset_data,k=4) + abline(h = 30, col = "red", lty = 2)

#db_test <- dbscan(subset_data,eps = 40)

#out <- kNNdist(subset_data, k = 4)
#print(kNNdistplot(subset_data, k = 4))

img <- png::readPNG('DataBsC/heatmaps_scanpaths_images/screenshot_fig1.png')
g <- grid::rasterGrob(img, interpolate = T)

#test_plot <- ggplot(test_data, aes(x = PositionX, y = PositionY)) +
#   xlim(0,1600) +
#  ylim(0,900)+ 
#  annotation_custom(g, xmin = 0, xmax = 1600, ymin = 0, ymax = 900) + geom_point(color = 'red') +
#  ggtitle("'The street performers in the night' by Marc Chagall")


subset_data <- as.matrix(subset(averaged_full_data_C0,  averaged_full_data_C0$image == "fig_12", select = c(PositionX, PositionY)))
plot  <-  fviz_cluster(db_fig_12, subset_data, stand = FALSE, geom = 'point', main = "'The Family' by Egon Schiele") +
  xlim(0,1600) +
  ylim(0,900)
