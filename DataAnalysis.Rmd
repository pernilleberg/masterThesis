...
#Testing on data from Davies et al.
mag_data <- read.csv("Gallery-study-1.0.0/MAG/data/condition1.csv")
mag_data <- mag_data[mag_data$GazeEventType == "Fixation", ] #Only fixations
participants <- as.vector(mag_data[!duplicated(mag_data$ParticipantName), 1])
#Get data for one specific stimulus only: mag_data[mag_data$MediaName == stimuli_name, ]
...
sparseness_df <- data.frame(ID = numeric(),
                            L1 = numeric(),
                            L2 = numeric(),
                            median_sal = numeric(),
                            mean_sal = numeric(),
...
temp = list.files(path = "salmaps/", pattern="*.csv", full.names = T)
myfiles = lapply(temp,read.delim,sep=",",header = F)

N = 1440000 #number of pixels
...
for (map in myfiles){
  ...
  sparseness_df[nrow(sparseness_df) + 1,] = c(ID = ID, L1 = L1, L2 = L2, median_sal = median_sal_val, mean_sal = mean_sal_val, sparseness = sparseness_val)
  ID = ID + 1
}

sparseness_df$artist <- c(rep("Chagall", 5), rep("Pollock", 5),
                                  rep("Rothko", 5), rep("Schiele", 5))
...
few_peak <- subset(sparseness_df,sparseness_df$sparseness >= 0.3981822)
many_peak <- subset(sparseness_df,sparseness_df$sparseness < 0.3981822)

mean(sparseness_df$mean_sal)

#Which value to properly display the many_peak/few_peak?

#write.csv(sparseness_df, 'sparseness_test.csv')
